...
class BasicBBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, norm_layer=None):
        super(BasicBBlock, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        if groups != 1:
            raise ValueError('BasicBlock only supports groups=1')
...
class BResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(planes[3] * block.expansion, num_classes)
        self.logvar = nn.Linear(planes[3] * block.expansion, num_classes)
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, BBottleneck) or isinstance(m, Bottleneck):
                    nn.init.constant_(m.bn3.weight, 0)
                elif isinstance(m, BasicBBlock) or isinstance(m, BasicBlock):
                    nn.init.constant_(m.bn2.weight, 0)
...
def bresnet101(bb=False, pretrained=False, **kwargs):
    """Constructs a BResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    if bb:
        model = BResNet(BBottleneck, [3, 4, 23, 3], **kwargs)
    else:
        model = BResNet(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet101']), strict=False)
    return model


def bresnet50(bb=False, pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    ...
    """
    if bb:
        model = BResNet(BBottleneck, [3, 4, 6, 3], **kwargs)
    else:
        model = BResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:

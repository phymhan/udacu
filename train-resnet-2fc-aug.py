...
import resnet
from utils import makedir, print_options
import utils
...
parser.add_argument('--tgt-train-list', default='tar_train_list.txt', type=str, dest='tgt_train_list',
...
parser.add_argument('--gpus', type=str, default='0', help='gpu ids: e.g. 0  0,1,2, 0,2. use "" for CPU')
parser.add_argument('--bayesian', action='store_true')
parser.add_argument('--entropy', action='store_true')  # TODO: annealing
...
def main():
    ...
    visDA17_trainset = ImageClassdata(txt_file=args.src_gt_list, root_dir=args.src_root, transform=data_transforms['train'])
    visDA17_valset = ImageClassdata(txt_file=args.tgt_gt_list, root_dir=args.tgt_root, transform=data_transforms['val'])

    train_loader = torch.utils.data.DataLoader(visDA17_trainset, batch_size=args.batch_size, shuffle=True, **kwargs)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if len(args.gpus) > 0 and torch.cuda.is_available():
            save_checkpoint({
                'epoch': epoch + 1,
...
def train(train_loader, model, optimizer, scheduler, epoch, device):
    ...
    for i, (input, label) in enumerate(train_loader):
        ...
        prec1 = accuracy(output.data, label, args.num_class, topk=(1,))[0]
        # print('=' * 50)
        # print(prec1[0])
        # print('=' * 50)
    ...
    if args.tensorboard:
        ...
        log_value('train_acc', top1.vec2sca_avg, epoch)
        log_value('lr', clr, epoch)
...
def train_bayes(train_loader, model, optimizer, scheduler, epoch, device):
    ...
    for i, (input, label) in enumerate(train_loader):
        ...
        # measure accuracy and record loss
        prec1 = accuracy(y_softmax.data, label, args.num_class, topk=(1,))[0]
        losses.update(loss, input.size(0))
        top1.update(prec1[0], input.size(0))
...
def RegCrossEntropyLoss_bayes(softmax, labels, mr_weight_l2, mr_weight_negent, mr_weight_kld, num_class):
    ...
    logsoftmax = torch.log(softmax + 1e-32)  # compute the log of softmax values
